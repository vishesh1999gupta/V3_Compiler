...
class Parser:
    ...
    def program(self):
        global indent
        self.emitter.headerLine("import java.util.*;")
        ...
        for i in range(indent):
                self.emitter.emit('    ')
        self.emitter.emitLine("public static void main(String[] args){")
        indent += 1
        for i in range(indent):
                self.emitter.emit('    ')
        self.emitter.emitLine("Scanner scn = new Scanner(System.in);")
    ...
    def statement(self):
        ...
        if self.checkToken(TokenType.PRINT):
            ...
            if self.checkToken(TokenType.STRING):
                ...
                # System.out.println();
                self.emitter.emitLine("System.out.print(\"" + self.curToken.text + "\\n\");")
                self.nextToken()
        ...
        # "FUNC" ident "() DO" nl {statement} "ENDFUNC" nl
            ...
            for i in range(indent):
                self.emitter.emit('    ')
            self.emitter.emit("public static void ")
        ...
        # "INPUT" ident
            ...
            for i in range(indent):
                self.emitter.emit('    ')
            self.emitter.emitLine(self.curToken.text +" = scn.nextFloat();")
            self.match(TokenType.IDENT)
